...
class CustomTokenizer(AbstractGazetteer):
    def __init__(self):
        ...
        self.regex_rules = [
            ...
            (re.compile(r'(?<!\d)([1234]?\d)\s?\/\s?52'), "Numeric", {"type": "weeks", "value": "assign_to_group_1"}),
            (re.compile(r'(?<!\d)([1234]?\d)(?:\+[1-7])?\s?\/\s?40'), "Numeric", {"type": "weeks", "value": "assign_to_group_1"}),
            (re.compile(r'((?:\d+)\/(?:\d+))'), "Numeric", {"type": "fraction", "value": "assign_to_group_1"}),
            (re.compile(r'((?:[0-9]*[.])?[0-9]+(?:[eE^][-+]?[0-9]+)?)'), "Numeric", {"value": "assign_to_group_1"}),
